...
stitched_image = false( 2*y_height, total_width); % Das leere Bild wird erstellt
width_covered=0;
y_shift_total = 0;
for range = length(Names):-1:1
    % Read the Last Image as the start image
    filename = strcat(directory_crop,'\', Names(range).name);
    reference_image = imread(filename);
    [y1, x1] = size(reference_image);
    if(range == length(Names)) %Das Letzte Bild wird nach vorne eingesetzt
        for x_sweep = 1:x1
            yy=0;
...
%search for white line of open end of stitched image
%     filename = strcat(directory_crop,'\', Names(range).name);
%     test_image = imread(filename);
...
%gefunden und im linken Teil des dazukommenden Bildes
    ...
    for x_sweep = StartPiRow:x1
            for y_sweep = 1:y1
                if(y_sweep+y_shift_total >=1 && y_sweep+y_shift_total <= 2*y1)
